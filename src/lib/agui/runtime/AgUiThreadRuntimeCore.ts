...
import type { AgUiEvent, AgUiRuntimeAgent } from "./types";
...
export class AgUiThreadRuntimeCore {
  private agent: AgUiRuntimeAgent;
  ...
  private async startRun(
    ...
    try {
      ...
      console.log("[AgUiCore] >>> Calling runAgent...");
      await (this.agent as any).runAgent(input, subscriber, {
        signal: abortSignal,
      });
      console.log("[AgUiCore] >>> runAgent COMPLETED");

      // @ag-ui/client sometimes resolves without emitting finalization events,
      // so ensure the subscriber sees a RUN_FINISHED.
      if (subscriber.onRunFinalized) {
        console.log("[AgUiCore] >>> Triggering onRunFinalized");
        subscriber.onRunFinalized();
      }
    } catch (error) {
      console.log("[AgUiCore] >>> runAgent ERROR:", error);
      ...
      console.log("[AgUiCore] >>> runAgent FINALLY block");
      this.finishRun(abortController);
    }
  ...
  private setRunning(running: boolean) {
    console.log("[AgUiCore] >>> setRunning:", running);
    this.isRunningFlag = running;
    this.notifyUpdate();
  }

  private finishRun(controller: AbortController | null) {
    console.log("[AgUiCore] >>> finishRun called");
